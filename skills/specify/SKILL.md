...
4. **Draft the `SPEC.md` change** (no commit yet):
   ...
   - **Stay strictly within THIS issue's criteria.** Model only what the acceptance criteria require — add no `Request`/`Decision` field, `outcome`, rule, or invariant borrowed from *another* story. Concretely, for a "take an order" story do **not** introduce pricing/`price`, a JSON ticket schema, or allergen handling: each is its own story. When tempted, leave it out and note the omission at review. Prefer the simplest model that satisfies the criteria, and keep reference data (a menu, prices) as an *external reference* the engine consults — do not hard-code a concrete menu into the spec.
...
- **Never widen scope beyond the criteria.** Add no field, `outcome`, rule, or invariant that THIS issue's acceptance criteria do not require — pricing, ticket/JSON schemas, allergen handling and the like each belong to their own story. If the criteria are ambiguous, ask; do not invent.
