...
3. **Map to `SPEC.md`**: propose the precise edit(s) to the spec that capture this behavior (a new rule, a precedence change, a contract change). Do **not** treat the GitHub prose as canonical — the spec is.
...
## Example
`/read-user-story #124` → "Contractors always REVIEW. SPEC.md: add R4, set precedence R2 > R4 > R1 > R3. Tests: (50, office, contractor) → REVIEW, ["R4"]; …"
